...
### 2.2 Фронтенд
На вашем компьютере соберите production-build:
```bash
cd frontend
...
yarn build
```
Загрузите **содержимое** папки `frontend/build/` в `public_html/`.
Также положите туда `.htaccess` из `cpanel/frontend_public_html/.htaccess` (он обрабатывает SPA-маршруты и кэш).
